...
export function mountDashboardApp({ root, fetchImpl = window.fetch.bind(window), apiUrl = "http://localhost:4000/api/recommendations", mockRides = defaultMockRides, mockLeads = defaultMockLeads }) {
    root.innerHTML = `
  ...
  `;
    const importRidesBtn = getRequiredElement(root, "#import-rides");
    const importLeadsBtn = getRequiredElement(root, "#import-leads");
    const runAnalysisBtn = getRequiredElement(root, "#run-analysis");
    const importState = getRequiredElement(root, "#import-state");
    const analysisStatus = getRequiredElement(root, "#analysis-status");
    const kpiGrid = getRequiredElement(root, "#kpi-grid");
    const actionFeedback = getRequiredElement(root, "#action-feedback");
    const actionList = getRequiredElement(root, "#action-list");
    const state = {
        ridesImported: false,
        leadsImported: false,
        analysisState: "idle",
    ...
    actionList.addEventListener("click", (event) => {
        ...
        renderActionList(actionList, state.analysisResult?.actions ?? [], state.appliedActionIds, state.analysisResult !== null);
...
function isAnalysisReady(state) {
    return state.ridesImported && state.leadsImported;
}
function getImportStateMessage(state, ridesCount, leadsCount) {
...
function getAnalysisStatusMessage(state, ridesCount, leadsCount) {
...
function renderActionList(container, actions, appliedActionIds, hasAnalysis) {
    if (actions.length === 0) {
        container.innerHTML =
            hasAnalysis
